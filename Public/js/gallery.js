...
function deleteZoomedPhoto(event) {
  ...
  const img = document.getElementById("lightbox-img");
  const relativePath = img.src.split(
    "/one-sided/Resources/gallery/uploads/"
  )[1];
  if (!relativePath) {
    alert("Could not determine image path.");
    return;
  ...
  fetch("delete_photos.php", {
    ...
    body: JSON.stringify({
      filenames: ["one-sided/Resources/gallery/uploads/" + relativePath],
    }),
  })
    .then((response) => response.json())
    .then((data) => {
      if (data.deleted && data.deleted.length > 0) {
        ...
        const imgElement = document.querySelector(
          `img[src="/one-sided/Resources/gallery/uploads/${relativePath}"]`
        );
        if (imgElement) {
          const figure = imgElement.closest("figure");
          if (figure) {
